...
def add_models_args(parser):
    ...
    parser.add_argument(
        "--epochs", type=int, default=10, help="num epochs to train for"
    )
...
class Seq2SeqSemanticParser(nn.Module):
    ...
    def encode_input(self, x_tensor, inp_lens_tensor):
        ...
        (enc_output_each_word, enc_context_mask, enc_final_states) = (
            self.encoder.forward(input_emb, inp_lens_tensor)
        )
        enc_final_states_reshaped = (
            enc_final_states[0].unsqueeze(0),
            enc_final_states[1].unsqueeze(0),
        )
...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(
        self, input_dim: int, full_dict_size: int, embedding_dropout_rate: float
    ):
        """
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, embedded_words, input_lens):
        ...
        packed_embedding = nn.utils.rnn.pack_padded_sequence(
            embedded_words, input_lens, batch_first=True, enforce_sorted=False
        )
...
def make_padded_input_tensor(
    exs: List[Example], input_indexer: Indexer, max_len: int, reverse_input=False
) -> np.ndarray:
    """
    ...
    if reverse_input:
        return np.array(
            [
                [
                    (
                        ex.x_indexed[len(ex.x_indexed) - 1 - i]
                        if i < len(ex.x_indexed)
...
def train_model_encdec(
    ...
    all_train_input_data = make_padded_input_tensor(
        train_data, input_indexer, input_max_len, reverse_input=False
    )
    all_test_input_data = make_padded_input_tensor(
        dev_data, input_indexer, input_max_len, reverse_input=False
    )

    output_max_len = np.max(np.asarray([len(ex.y_indexed) for ex in train_data]))
    all_train_output_data = make_padded_output_tensor(
        train_data, output_indexer, output_max_len
    )
    all_test_output_data = make_padded_output_tensor(
        dev_data, output_indexer, output_max_len
    )

    if args.print_dataset:
        print("Train length: %i" % input_max_len)
        print(
            "Train output length: %i"
            % np.max(np.asarray([len(ex.y_indexed) for ex in train_data]))
        )
        print(
            "Train matrix: %s; shape = %s"
