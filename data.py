...
class Example(object):
    ...
    def __init__(
        self, x: str, x_tok: List[str], x_indexed: List[int], y, y_tok, y_indexed
    ):
        self.x = x
        self.x_tok = x_tok
...
def index(x_tok: List[str], indexer: Indexer) -> List[int]:
    return [
        (
            indexer.index_of(xi)
            if indexer.index_of(xi) >= 0
            else indexer.index_of(UNK_SYMBOL)
        )
        for xi in x_tok
    ]
...
def print_evaluation_results(
    test_data, selected_derivs, denotation_correct, example_freq=50, print_output=True
):
    """
    ...
    return [
        num_exact_match / len(test_data),
        num_tokens_correct / total_tokens,
        num_denotation_match / len(test_data),
    ]
